...
class attention(nn.Module):
    def __init__(self, dim, num_head=8, window=7, norm_cfg=dict(type="SyncBN", requires_grad=True), drop_depth=False):
        ...
        # LEA (RGB) extra layers to mirror depth LEA: Linear(C->C/2) -> DWConv7x7 -> Linear(C/2->C/2)
        self.q_conv = nn.Conv2d(dim // 2, dim // 2, kernel_size=7, padding=3, groups=dim // 2)
        self.q_back = nn.Linear(dim // 2, dim // 2)
    ...
    def forward(self, x, x_e):
        ...
        q = self.q(x)
        q_lea = self.q_cut(x)                                        # (B,H,W,C/2)
        q_lea = self.q_conv(q_lea.permute(0, 3, 1, 2))               # NCHW for conv
        q_lea = q_lea.permute(0, 2, 3, 1)                            # back to NHWC
        q_lea = self.q_back(q_lea)                                   # (B,H,W,C/2)
